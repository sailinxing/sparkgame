...
object GameKpi {
  def main(args: Array[String]): Unit = {
    ...
    //切分之后的数据
    val splitedLogs = sc.textFile("D://testsparkdata//game//GameLog.txt").map(_.split("\\|"))
    //过滤后并缓冲
    ...
    val todayLoginUser = filteredLogs.filter(fields => FilterUtils.filterByType(fields, EventType.LOGIN)).map(x => (x(3), 1)).distinct()
